...
void task_5() {
...

    {
        ...
        std::cout << "1) " << "x = 3 + 4 + 5 = " << x + 3 + 4 + 5 << '\n'; // выводится сумма значения переменной х и трёх числовых литералов
        std::cout << "2) " << "x = y = x = " << (x == y == z) << '\n'; // происходит сравнение на равенство значений указанных переменных

        // в таком выражении можно запутаться с ассоциативностью
        z *= ++y + 5;
        std::cout << "3) " << "z *= ++y + 5 = " << z << '\n';

        bool logicValue;
        // чтобы logicValue имела значения false, необходимо, чтобы все используемые переменные имели значение 0, ведь данные переменные имеют тип int
        // либо, чтобы одно из выражений (x || y или z || v) вернуло false
        // операция && имеет больший приоритет, чем ||
        logicValue = x || y && z || v;
        std::cout << "4) logicValue = x || y && z || v = " << logicValue << '\n';
    }
